...
async def register_player(quiz_id: int, player_id: str, display_name: str) -> dict:
    def _op():
...
async def list_registrations(quiz_id: int) -> list:
    def _op():
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
...
async def get_registered_quiz_ids(player_id: str) -> set:
    def _op():
        with get_connection() as conn:
            with conn.cursor() as cur:
...
async def get_finished_quiz_ids(player_id: str) -> set:
    def _op():
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
...
async def list_player_attempts(player_id: str) -> list:
    def _op():
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
...
async def finish_stale_attempts(quiz_id: int):
...
async def get_or_create_attempt(quiz_id: int, player_id: str) -> dict:
    def _op():
        with get_connection() as conn:
            with conn.cursor() as cur:
...
async def get_attempt(quiz_id: int, player_id: str) -> dict | None:
    def _op():
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
...
async def save_answer(attempt_id: int, question_index: int, selected_index: int, is_correct: bool, time_ms: int) -> dict:
    def _op():
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
...
async def update_attempt_progress(attempt_id: int, score: int, total_time_ms: int, question_index: int, status: str = "playing"):
    def _op():
        with get_connection() as conn:
            with conn.cursor() as cur:
...
async def count_attempts(quiz_id: int) -> dict:
    def _op():
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
...
async def get_display_name(quiz_id: int, player_id: str) -> str:
    def _op():
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
