...
async def register(
    ...
    user_id = await asyncio.to_thread(
        create_keycloak_user, username, email, first_name, last_name, password, role
    )
...
async def login(email: str = Body(...), password: str = Body(...)):
    return await asyncio.to_thread(login_user, email, password)


@router.post("/logout")
async def logout(refresh_token: str = Body(..., embed=True)):
    return await asyncio.to_thread(logout_user, refresh_token)
