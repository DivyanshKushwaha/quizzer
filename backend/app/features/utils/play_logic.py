...
def list_quizzes(player_id=None):
    rows = quiz_db.list_quizzes(audience="player")
    registered = play_db.get_registered_quiz_ids(player_id) if player_id else set()
    played = play_db.get_finished_quiz_ids(player_id) if player_id else set()
    visible = []
    for q in rows:
        if q["id"] in played:
            continue
...
def my_attempts(player_id):
    return play_db.list_player_attempts(player_id)


def register(quiz_id, player_id, body):
    name = body.get("display_name") or player_id[:8]
    row = play_db.register_player(quiz_id, player_id, name)
    redis_db.publish(quiz_id, {"type": "lobby"})
    redis_db.publish_global({"type": "feed"})
    return row


def lobby(quiz_id, player_id=None):
    is_registered = bool(player_id) and quiz_id in play_db.get_registered_quiz_ids(player_id)
    return {
        "quiz": quiz_db.get_quiz(quiz_id, audience="player"),
        "registered": play_db.list_registrations(quiz_id),
        "is_registered": is_registered,
    }


def play_state(quiz_id, player_id, join=False):
    if join:
        attempt = play_db.get_or_create_attempt(quiz_id, player_id)
        if attempt.get("_created"):
            redis_db.incr_playing(quiz_id)
    else:
        attempt = play_db.get_attempt(quiz_id, player_id)
        if not attempt:
    ...
    if not done and idx < len(questions):
        ...
        if timer_sec:
            remaining = redis_db.set_question_deadline(quiz_id, player_id, idx, int(timer_sec))
...
def submit_answer(quiz_id, player_id, body):
    selected = body.get("selected_index")
    if selected is None:
        raise AuthForbidden("selected_index required")

    quiz = quiz_db.get_quiz(quiz_id, include_answers=True)
    attempt = play_db.get_attempt(quiz_id, player_id)
    ...
    if not redis_db.try_idempotent(attempt["id"], q_idx):
        return play_state(quiz_id, player_id)
    ...
    within_deadline = redis_db.is_deadline_alive(quiz_id, player_id, q_idx)
    ...
    if not play_db.save_answer(attempt["id"], q_idx, selected, is_correct, time_ms):
        return play_state(quiz_id, player_id)
    ...
    play_db.update_attempt_progress(attempt["id"], new_score, new_time, next_idx, status)
    redis_db.update_leaderboard(quiz_id, player_id, quiz_logic.encode_rank_score(new_score, new_time), {
        "score": new_score,
        "question_index": next_idx,
        "display_name": play_db.get_display_name(quiz_id, player_id),
    })
    if finished:
        redis_db.incr_finished(quiz_id)

    return {**play_state(quiz_id, player_id), "score_delta": points}


def leaderboard(quiz_id, player_id=None, top=False):
    rows = redis_db.get_top_leaderboard(quiz_id) if top else redis_db.get_leaderboard_window(quiz_id, player_id)
    return [{**r, "display_name": play_db.get_display_name(quiz_id, r["player_id"])} for r in rows]


def presence(quiz_id):
    sample = redis_db.get_presence_sample(quiz_id)
    counts = play_db.count_attempts(quiz_id)
    sample["playing"] = counts["playing"]
    sample["finished"] = counts["finished"]
    return sample


def result(quiz_id, player_id):
    quiz = quiz_db.get_quiz(quiz_id)
    ...
    rank = redis_db.get_player_rank(quiz_id, player_id)
    if not rank:
        return {
            "rank": 0,
            "score": attempt["score"],
